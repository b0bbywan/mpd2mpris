...
async def test_cover_url_returns_image(monkeypatch) -> None:
    ...
    monkeypatch.setattr(mympd, "_post", _post)
...
async def test_cover_url_network_error(monkeypatch) -> None:
    ...
    monkeypatch.setattr(mympd, "_post", _boom)
    assert await mympd.cover_url("http://host:8080", "http://stream") is None
