...
from mpd2mpris import radiobrowser
...
def _favicons_alive(monkeypatch):
    ...
    monkeypatch.setattr("mpd2mpris._http.is_image", lambda url: True)
...
async def test_station_icon_returns_favicon_url(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mpd2mpris._http.get", _get)
...
async def test_station_icon_no_station(monkeypatch) -> None:
    monkeypatch.setattr("mpd2mpris._http.get", lambda url: b"[]")
    assert await radiobrowser.station_icon("http://stream") is None


@pytest.mark.asyncio
async def test_station_icon_no_favicon(monkeypatch) -> None:
    monkeypatch.setattr("mpd2mpris._http.get", lambda url: json.dumps([{"name": "X", "favicon": ""}]).encode())
    assert await radiobrowser.station_icon("http://stream") is None
...
async def test_station_icon_literal_null_string(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mpd2mpris._http.get", lambda url: json.dumps([{"name": "X", "favicon": "null"}]).encode())
    assert await radiobrowser.station_icon("http://stream") is None
...
async def test_station_icon_skips_dead_favicon(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mpd2mpris._http.get", lambda url: payload)
    monkeypatch.setattr("mpd2mpris._http.is_image", lambda url: url != "https://dead/favicon.ico")
...
async def test_station_icon_skips_non_image_favicon(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mpd2mpris._http.get", lambda url: payload)
    monkeypatch.setattr("mpd2mpris._http.is_image", lambda url: False)
    assert await radiobrowser.station_icon("http://stream") is None


@pytest.mark.asyncio
async def test_station_icon_non_list_payload(monkeypatch) -> None:
    # An error/HTML response (a JSON object, not the station array) must be a
    # clean miss, not an AttributeError from iterating dict keys.
    monkeypatch.setattr("mpd2mpris._http.get", lambda url: b'{"message": "rate limited"}')
    assert await radiobrowser.station_icon("http://stream") is None


@pytest.mark.asyncio
async def test_station_icon_transient_is_image_error_falls_through(monkeypatch) -> None:
    # A transient HEAD failure on the first favicon must not abandon the scan;
    # the next station's valid favicon is still returned.
    payload = json.dumps([
        {"name": "Flaky", "favicon": "https://flaky/favicon.ico"},
        {"name": "Live", "favicon": "https://live/favicon.ico"},
    ]).encode()
    monkeypatch.setattr("mpd2mpris._http.get", lambda url: payload)

    def _is_image(url: str) -> bool:
        if url == "https://flaky/favicon.ico":
            raise OSError("transient")
        return True

    monkeypatch.setattr("mpd2mpris._http.is_image", _is_image)
    assert await radiobrowser.station_icon("http://stream") == "https://live/favicon.ico"


@pytest.mark.asyncio
async def test_station_icon_all_is_image_errors_propagate(monkeypatch) -> None:
    # No usable favicon AND a transient error → propagate so cover.py skips
    # caching and retries, rather than caching a false miss.
    payload = json.dumps([{"name": "Flaky", "favicon": "https://flaky/favicon.ico"}]).encode()
    monkeypatch.setattr("mpd2mpris._http.get", lambda url: payload)
...
async def test_station_icon_network_error_propagates(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mpd2mpris._http.get", _boom)
    with pytest.raises(OSError):
        await radiobrowser.station_icon("http://stream")
