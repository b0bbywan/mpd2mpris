...
async def test_resolve_album_unparseable_title_skips_query(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mpdris2._http.get", _router({"never": b""}))
...
@pytest.mark.asyncio
async def test_resolve_album_network_error_propagates(monkeypatch) -> None:
    monkeypatch.setattr("mpdris2._http.get", _router({"search/track": OSError("boom")}))
    with pytest.raises(OSError):
        await deezer.resolve_album("A - B")
