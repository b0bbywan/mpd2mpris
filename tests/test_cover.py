...
def test_cue_dir_from_song_file_uses_grandparent(tmp_path: Path) -> None:
    album = tmp_path / "Artist"
    album.mkdir()
    (album / "playlist.cue").touch()
    cf = CoverFinder(CoverFinderConfig(music_dir=tmp_path))
    assert cf._cue_dir_from_song_file(
        "Artist/playlist.cue/track0001"
    ) == Path("Artist")


def test_cue_dir_from_song_file_real_dir_parent_returns_none(tmp_path: Path) -> None:
    # Regular track ("Artist/Album/track.flac"): the parent is a dir,
    # not a virtual-CUE container — leave it for the normal step 2.
...
def test_cue_dir_from_song_file_no_music_dir_returns_none() -> None:
    # Without music_dir we can't check whether the parent is a file.
    cf = CoverFinder(CoverFinderConfig())
    assert cf._cue_dir_from_song_file("Artist/playlist.cue/track0001") is None
...
async def test_cue_fallback_infers_from_song_file_when_playlist_empty(
    ...
    # ``load`` — adding it through ``add`` leaves the field empty. In
    # that case derive the cue dir from ``song_file`` itself: a virtual
    # track ``dir/playlist.cue/trackNNNN`` whose parent is a real file
    # means the grandparent holds the cover.
    album_dir = tmp_path / "GrosseRadioReggae"
    album_dir.mkdir()
    (album_dir / "playlist.cue").touch()
