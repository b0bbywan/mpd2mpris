...
| `daemon.py` | `run()` coroutine — MPD connect/reconnect, D-Bus export, MPRIS callbacks, idle-driven `refresh()` |
...
- **Systemd**: two unit files in `data/{system,user}/mpDris2.service` with `Restart=always RestartSec=5` and `ConditionUser=!root` / `ConditionUser=!@system`. Neither is auto-enabled on install.
...
- mmkeys-related config keys (`[Bling] mmkeys`, `[Bling] cdprev`) are no longer honoured.
