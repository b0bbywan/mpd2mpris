...
class CoverFinder:
    ...
    def _resolve_cue_dir(self, req: SongLookup) -> Path | None:
        ...
        ``song_file`` itself."""
        return (
            self._cue_dir_from_playlist(req.last_loaded_playlist)
            or self._cue_dir_from_song_file(req.song_file)
        )
    ...
    def _cue_dir_from_song_file(self, song_file: str) -> Path | None:
        """A virtual track has the form
        ``dir/container.{cue,flac,…}/trackNNNN``. When the parent path
        component points to a real file under ``music_dir`` it's a
        container and the grandparent is the cue dir — needed when
        ``lastloadedplaylist`` is empty (CUE added via ``add`` rather
        than ``load``)."""
        if not song_file or _has_uri_scheme(song_file) or not self._music_dir:
            return None
        container = Path(song_file).parent
        if str(container) in ("", "."):
            return None
        if not (self._music_dir / container).is_file():
            return None
    ...
    def _lookup_downloads_cache(self, mpd_meta: dict) -> str | None:
        ...
        safe_name = f"{artist_key}-{album}.jpg".replace("/", "_")
