...
def _norm(s: str) -> str:
    ...
    # "Telephone" == "Téléphone"), then delegate the lowercase/alnum
    # collapse to translate.normalize.
...
def _resolve_blocking(q_artist: str, q_track: str) -> tuple[str, str] | None:
    ...
    # jingle or a loose coincidence never yields a cover. A clean miss
    # returns None; a network/transient error propagates so the caller can
...
def _url_blocking(artist: str, album: str) -> str | None:
    ...
    try:
        ...
        # ResponseError carries the HTTP status in ``.cause``. A 404 is a
        # confirmed "no cover art" (cache it as a miss); anything else
        # (e.g. 503 rate-limiting, network) is transient — propagate so the
        # caller skips caching and retries.
