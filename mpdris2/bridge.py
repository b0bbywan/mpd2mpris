...
class MpdMprisBridge:
    ...
    def __init__(
        ...
        *, bus: MessageBus, notifier: Notifier | None = None,
    ) -> None:
    ...
    async def refresh(self) -> None:
        ...
        if self.notifier and not same_song and notify_state:
            self._schedule(self.notifier.notify_track(
                meta, state, int(new_pos_s * 1_000_000),
            ))
    ...
    async def run_loop(self) -> None:
        ...
        while True:
            ...
            if self.music_dir is None:
                logger.warning(
                    "no music library configured; cover art and file:// URIs unavailable",
                )
